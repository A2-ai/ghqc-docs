...
`ghqc` utilizes GitHub as its *hub* for QC, centralizing files and their traceability in one easily accessible, persistent location.  
In addition to the GitHub Issue storing the history of the QC, `ghqc` can create a PDF report to [record the QC process](../record) and an [archive](../archive) to store files as well as metadata related to those files.
