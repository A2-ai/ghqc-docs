...
description: An index page describing the ghqc workflow
---
...
* **Comments** - This feature provides an intuitive way for reviewers and authors to communicate, 
...
<iframe
    ...
    style="width: 100%; height: 67.5vh; border: none;"
>
</iframe>

### GitHub Milestones

Milestones serve as the organizational unit of the QC. Milestones group related issues under common goals or timelines, such as a project phase.
...
2. **Review** - After initialization, [review](../review) occurs. `ghqc` promotes an iterative and interactive method by allowing the QCers to comment within the
Issue and the author to reply back with a file difference, highlighting the changes, using the [`ghqc` notify app](../notify).
3. **Approval** - After the QCer has reviewed the file and has all of their feedback addressed, the Issue can be [approved and closed through the `ghqc` status app](../approve).

After QC the author can create an exportable [record](../../intro/record) of what occurred within GitHub.

This is visualized below:

![](../../../../assets/legacy/workflow_summary.png)

On the following pages, we will go into detail of how ghqc aids users to work through the QC workflow for a file, 
from how the author creates a milestone populated with issues to approval.
