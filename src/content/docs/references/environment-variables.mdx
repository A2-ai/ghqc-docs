...
| `GHQC_ALLOW_CONFIG_UPDATE` | Boolean | `true` |
...
Whether the web UI may fast-forward the configuration repository onto its remote.
...
Accepted spellings are case-insensitive: `true`/`false`, `1`/`0`, `yes`/`no`, `on`/`off`.

Unrecognized values fall back to the next source — and therefore to the default `true` — rather than disabling updates.
A typo such as `GHQC_ALLOW_CONFIG_UPDATE=flase` can never silently switch updates off.

When the resolved value is `false`, the Configuration tab renders no **Update** button, the nav tab's staleness badge is suppressed, and `POST /api/configuration/update` returns `403`.
The `ghqc configuration update` CLI command is deliberately unaffected.
