...
It combines binary information, repository status, and configuration details in one command, which makes it useful for troubleshooting or sharing setup context in a bug report.
...
The text report is divided into three sections:
...
If repository inspection fails, `sitrep` still prints the binary and configuration sections and replaces the repository block with an error message.
...
<LinkCard title="Configuration Status" href="../configuration/status" description="Use the narrower configuration-only report when you do not need repository or binary details"/>
